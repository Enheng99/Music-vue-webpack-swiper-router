...
$('.main_all').on('click','.main_list_text input',function(){
	...
	var html='';
	html+='<li>';
    html+='<em><a href="##">叮咚：</a></em>';
    html+='<p>'+text+'</p>';
    html+='<span><a class="Js_talk_del" href="javascript:void(0)">删除</a><a  class="Js_talk_res" href="javascript:void(0)">回复</a></span></li>';
...
$('.main_all').on('click','.main_detail_imgImg',function(){
	...
	if(imgW<=200)
	{
		$(this).css({float:'none',width:'auto',display:'block'});
	}
	else
	{
		$(this).css({float:'left',width:'200px',display:'block'});
	}
...
//视频播放
$('.main_all').on('click','.main_detial_videoImg',function(){
...
$('.main_all').on('click','.main_v_xx',function(){
	...
	$(this).parents('.main_detail_videoM').siblings('.main_detial_videoImg').show();
});
...
$('.main_all').on('click','.main_detail_images',function(){
	...
	if(liW==100)
	{
		$(this).find('li').css({float:'none',width:'100%',height:'auto'});
		$(this).find('img').css({width:'auto'});
	}
	else
	{
		$(this).find('li').css({float:'left',width:'100px',height:'100px'});
...
$(window).scroll(function(){
	...
	var webH=$('html').height();//网页实际高度
	var scrH=$('body').scrollTop()+$('html').scrollTop();//body支持chrome等；html支持IE与FF;滚动高度
	//alert(winH+'  '+webH+'  '+scrH);
	if(scrH>=webH-winH)
	{
